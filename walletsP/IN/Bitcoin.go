	...
	for _, checkAddress := range address {
		for userID, P_address := range PendingAdderess.data {
			if checkAddress == P_address {
				addToBalance(1, uint32(userID), uint64(funds*1000000000)) //0,0001
			}
		}
	}
...
func saveLastBlock(block string) {
	...
	f, err := os.OpenFile("../Bitcoin/lastBTCBlock.log", os.O_CREATE|os.O_WRONLY, 0644)
	if err != nil {
		log.Fatal(err)
	}
...
func sendColdWalletData(Tx *btcjson.TxRawResult,  Vout uint64) {
	bufOUT := make([]byte, 80)
	binary.LittleEndian.PutUint64(bufOUT[0:8], 1)
...
func save_Transaction(Tx *btcjson.TxRawResult, TrTo string, value float64, userID uint64, Vout uint64, isLoad int) {
	...
	TrValue := fmt.Sprintf("%v", (value*1000000000))
	if TrTo == ColdWallet {
		...
		for _, TranXon := range PendingTransactionsTMP {
			...
			if status == true {
				...
				if usersTrunsactionsT.data[TranXon.Hash].UserId != 0 && len(usersTrunsactionsT.data[TranXon.Hash].TransactionHash) > 10 {
					...
					//searchInByteArray(TMPTransactionLIST, buf, usersTrunsactionsT.data[TranXon.Hash])
...
func load_trunsuction_to(filename string, tr_type string, client *rpcclient.Client) {
	...
	for i = 0; i < uint64(fi.Size()/(lenD)); i++ {
		...
		if tr_type == "PendingOUT" {
			...
			//PendingTransactionsOUT.data[transuction.Txid] = transuction
		} else {
			data = readNextBytes(file, int(lenD))
		}
		...
		if tr_type == "Pending" {
			//PendingTransactions.data[transuction.Txid] = transuction
...
test Function
 */
func searchInByteArray(data *transactions, buf []byte, trData UserTrunsaction) {
	j := 0
	var updateFlag bool
	updateFlag = false
	for n := 0; n < (len(data.transactionsHistory) / 192); n++ {
		uIDCMP := binary.LittleEndian.Uint64(data.transactionsHistory[0+j:8+j])
		InOutCMP := binary.LittleEndian.Uint16(data.transactionsHistory[8+j:10+j])

		TrToCMP := string(bytes.Trim(data.transactionsHistory[24+j:66+j], "\x00"))
		TrFromCMP := string(bytes.Trim(data.transactionsHistory[66+j:108+j], "\x00"))
		TransactionHashCMP := string(bytes.Trim(data.transactionsHistory[126+j:192+j], "\x00"))

		if uIDCMP == trData.UserId && InOutCMP == uint16(trData.InOut) && TrToCMP == trData.TrTo && TrFromCMP == trData.TrFrom && TransactionHashCMP == trData.TransactionHash {
			var tmpBuf= make([]byte, 2)
			binary.LittleEndian.PutUint16(tmpBuf[0:2], uint16(trData.ConfirmedBlocks))
			copy(data.transactionsHistory[14+j:16+j], tmpBuf)
			updateFlag = true
			break
		}
		j = j + 192
	...
	if updateFlag == false {
		data.transactionsHistory = append(data.transactionsHistory, buf...)
	}
}
...
func writeDataToFile(listTransactions []byte, userId uint16, dataFiles *Storage) (int) {
	...
	if (endFile != 0) {
		file, err := os.OpenFile(fmt.Sprintf("../Bitcoin/%vU%d.db", dataFiles.name, userId), os.O_WRONLY|os.O_CREATE, 0666) //TODO filter userid leave only numbers!!!
		if err != nil {
			log.Fatal(err)
		}
		recordData.file = 4
...
func readMapData(dataStorage *Storage) {
	...
	file, err := os.Open(fmt.Sprintf("../Bitcoin/%vMap.db",dataStorage.name))
...
func readFileData(userId uint64, amount uint64, offset uint64, dataFiles *Storage) ([]byte) {
	...
	if (rFile > 0) {
		fileX, err := os.Open(fmt.Sprintf("../Bitcoin/%vU%d.db", dataFiles.name, userId))
...
func main() {
	...
	ColdWallet = LocalConfig.BitcoinColdWallet
	CustomNetwork.DefaultPort = LocalConfig.BitcoinDefaultPort
	...
	err := sndBalance.Bind(fmt.Sprintf("%v%v", LocalConfig.LocalTcpIpAddr, LocalConfig.BTCBalancePort));
	if (err != nil){
		fmt.Println(err)
	}

	//Creating connection with Bitcoin OUT server (receive)
	//receive new transaction
	rcvOutTransaction, _ = zmq.NewSocket(zmq.PULL)
	defer rcvOutTransaction.Close()
	rcvOutTransaction.SetRcvhwm(1100000)
	rcvOutTransactionAddress := fmt.Sprintf("%v%v", LocalConfig.LocalTcpIpAddr, LocalConfig.BTCOutTrnsctnPort)
	...
	rcvAddress := fmt.Sprintf("%v%v", LocalConfig.LocalTcpIpAddr, LocalConfig.BTCrcvNewAddressPort)
	...
	sndTransactionHistoryAddress := fmt.Sprintf("%v%v", LocalConfig.LocalTcpIpAddr, LocalConfig.BTCsndTrnsctnHstrPort)
	err = sndTransactionHistory.Bind(sndTransactionHistoryAddress)
	checkError(err)

	//Creating connection with Bitcoin OUT  server (send)
	...
	sndTransactionHistoryAddressOUT := fmt.Sprintf("%v%v", LocalConfig.LocalTcpIpAddr, LocalConfig.BTCsndTrnsctnHstrOUTPORT)
	...
	sndColdTxInAddressOUT := fmt.Sprintf("%v%v", LocalConfig.LocalTcpIpAddr, LocalConfig.BTCsndColdTxInOUTPort)
	...
	//Read data from Bitcoin Transactions Storage
