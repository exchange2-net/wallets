...
var FooPicker = (function () {
    ...
    _self.showPicker = function() {
      ...
      if (pickerField) {
        ...
        if (pickerDiv) {
          ...
          pickerDiv.style.left = `calc(${right}px - 245px)`;
  ...
  function format(instance, day, month, year) {
    ...
    switch(instance.options.dateFormat) {
      ...
      case 'dd.MMM.yyyy':
        return day + '.' + getShortMonth(parseInt(month)) + '.' + year;
      case 'MMM dd, yyyy':
          return getShortMonth(parseInt(month)) + ' ' + day + ', ' + year;
      case 'dd/MM/yyyy':
        return day + '/' + month + '/' + year;
      case 'dd/MMM/yyyy':
        return day + '/' + getShortMonth(parseInt(month)) + '/' + year;
  ...
  function parse(instance, value) {
    ...
    switch(instance.options.dateFormat) {
      ...
      case 'MMM dd, yyyy':
        dateArray = value.split(' ');
        date = new Date(parseInt(dateArray[2]), getMonthNumber(dateArray[0]), parseInt(dateArray[1]));
        return date;
      case 'dd/MM/yyyy':
        dateArray = value.split('/');
        date = new Date(parseInt(dateArray[2]), parseInt(dateArray[1]) - 1, parseInt(dateArray[0]));
