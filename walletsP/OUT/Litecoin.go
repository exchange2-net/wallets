...
function returns address map in ltcutil format
*/
func createAddressMap(addresses map[string]uint64) map[ltcutil.Address]ltcutil.Amount {
	var addressMap = make(map[ltcutil.Address]ltcutil.Amount)
	defaultNet := CustomNetwork
	for address, amount := range addresses {
		c_address, err := ltcutil.DecodeAddress(address, defaultNet)
		if err != nil {
			fmt.Println(err)
		}
		addressMap[c_address] = ltcutil.Amount(amount)
	}
	return addressMap
}
...
args: client(connection to litecoin Core), user ID, Value, user Private Key
...
func CreateInputs(client *rpcclient.Client, userId uint64, Value uint64, PrivateKey string) ([]btcjson.TransactionInput, map[string]uint64, string, string) {
		...
		for _, TrData := range userTransactions { // range user' inputs
			if TrData.UserId == userId {
				...
				if DataValue < Value {
					...
					//add new input
					inputs = append(inputs, btcjson.TransactionInput{Txid:txHash, Vout:Vout})
					//continue searching
					continue
	...
	if (ValueCMP - Value) != ValueCMP { //if not enough money in user' inputs we will use cold  wallet inputs
		ColdKey = LocalConfig.LitecoinWalletPass // Private Key of cold Wallet
...
args: send coins to, amount of coins, Fee, client(connection to litecoin Core), sender private key, sender ID
function returns TxId or error
*/
func CreateRawTransaction(sendTo string, amount uint64, Fee ltcutil.Amount, client *rpcclient.Client, PrivateKey string, userId uint64) *chainhash.Hash {
	var addressMap = make(map[string]uint64)
	currentTime := int64(0)
	...
	if is_ok {
		...
		TxId,err := client.SendRawTransaction(SignTransaction, false)
		if err != nil {
			return nil
		}
...
function of sending transaction data to the Litecoin listening server…
*/
...
func listen_wallet(client  *rpcclient.Client) {
	...
	for {
		...
		if Tx == nil {
			...
			binary.LittleEndian.PutUint16(lineBuf[0:2], 3)
...
*/
func saveColdTxIn() {
	file, err := os.OpenFile("../Litecoin/ColdWalletsTxIn.db", os.O_CREATE|os.O_WRONLY, 0666)
	defer file.Close()
	if err != nil {
...
*/
func saveTxIn() {
	file, err := os.OpenFile("../Litecoin/UsersTxIn.db", os.O_CREATE|os.O_WRONLY, 0666)
	defer file.Close()
	if err != nil {
		log.Fatal("read file bans 1072",err)
	}
...
func loadColdWalletTxIn() {
	file, err := os.Open("../Litecoin/ColdWalletsTxIn.db")
	defer file.Close()
	if err != nil {
		log.Fatal(err)
	}
...
*/
func loadTxIn() {
	file, err := os.Open("../Litecoin/UsersTxIn.db")
	defer file.Close()
	if err != nil {
...
func main() {
	...
	//-----
	CustomNetwork.DefaultPort = LocalConfig.LitecoinDefaultPort // network Params
	ColdWallet = LocalConfig.LitecoinColdWallet //cold wallet address

	connCfg := &rpcclient.ConnConfig{ //Litecoin core connection config
	...
	err = sndBalance.Bind(fmt.Sprintf("%v%v", LocalConfig.LocalTcpIpAddr, LocalConfig.LTCBalanceOUTPort));
	checkError(err) //check for errors
	...
	rcvTransactionHistoryAddresses := fmt.Sprintf("%v%v", LocalConfig.LocalTcpIpAddr, LocalConfig.LTCrcvTrnsctnHstrOUTPort)
	...
	rcvColdTxInAddress := fmt.Sprintf("%v%v", LocalConfig.LocalTcpIpAddr, LocalConfig.LTCrcvColdTxInPort)
	...
	err = sndOutTransaction.Bind(fmt.Sprintf("%v%v", LocalConfig.LocalTcpIpAddr, LocalConfig.LTCsndOutTrnsctnPort))
	checkError(err) //check for errors

	sndTransactionAnswer, _ = zmq.NewSocket(zmq.PUSH)
	defer sndTransactionAnswer.Close()
	sndTransactionAnswer.SetRcvhwm(1100000)
	err = sndTransactionAnswer.Bind(fmt.Sprintf("%v%v", LocalConfig.LocalTcpIpAddr, LocalConfig.LTCsndTrnsctnAnswerPort))
	checkError(err) //check for errors

	rcvTransaction, _ = zmq.NewSocket(zmq.PULL)
	defer rcvTransaction.Close()
	rcvTransaction.SetRcvhwm(1100000)
	rcvAddress := fmt.Sprintf("%v%v", LocalConfig.LocalTcpIpAddr, LocalConfig.LTCrcvNewAddressOUTPort)
